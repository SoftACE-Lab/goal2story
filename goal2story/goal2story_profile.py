...
actor_system_prompt = " You are an experienced Product Owner who is good at identifying key actors that can influence the goal based on project information."
impact_system_prompt = "You are an experienced Business Analyst who is good at identifying key impacts related to the actor as well as the goal."
deliverable_system_prompt = " You are an experienced Scrum Master who is good at analyzing key deliverables, which are the specific features or activities implemented to support the required impacts."
userstory_system_prompt = "You are an experienced Product Owner who is good at identifying key user stories that describe an actor’s actions and the expected outcome within a system."
                    ...
                    while retry_count < max_retries and not success:
                        try:
                            ...
                            response = client.chat.completions.create(
                                ...
                                temperature=0.9,
                                messages=[
                                    {"role": "system", "content": deliverable_system_prompt},
                                    {"role": "user", "content": current_prompt}
                                ],
                ...
                while retry_count < max_retries and not success:
                    try:
                        ...
                        response = client.chat.completions.create(
                            ...
                            temperature=0.9,
                            messages=[
                                {"role": "system", "content": userstory_system_prompt},
                                {"role": "user", "content": current_prompt}
                            ],
                        )
                        cleaned_response = clean_json_response(response.choices[0].message.content)
