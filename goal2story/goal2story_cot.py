...
def read_prompt(file_name):
    ...
    with open(os.path.join('../prompts_cot', file_name), 'r', encoding='utf-8') as f:
        return f.read()
...
def fix_json_with_agent(json_content):
    ...
    try:
        ...
        response = client.chat.completions.create(
            model=model,
            temperature=0.1,  # Low temperature to ensure more deterministic output
            messages=[
...
def extract_json_content(text):
    """Extract the content between ```json and ``` in text"""
    try:
...
# Modify clean_json_response function, use formatting agent
...
# Check if there's an existing CSV file before starting the main loop
...
for row_index, csv_data in enumerate(csv_data_df.itertuples(index=False), 1):
    ...
    try:
        ...
        # Get the prompt template for single actor's impact
        single_impact_prompt = read_prompt('impact_prompt.txt')

        # Store all impacts results
        all_impacts = {
            "goal": goal,
            "actors": []
        }

        # Get impacts for each actor
        for actor_id, actor_info in all_actors["actors"].items():
            ...
            try:
                # Use build_prompt function to build prompt
                ...
                while retry_count < max_retries and not success:
                    try:
                        ...
                        # Build actor data structure in required format
                        actor_data = {
                            "actor_id": actor_id,
                            "actor_name": actor_info["name"],
                            "impacts": []
        ...
        # Print the final complete results
        print(f"\n{Fore.GREEN}Final combined impacts:")
        print(json.dumps(all_impacts, indent=2))
        print(Style.RESET_ALL)

        # Get the prompt template for deliverable
        deliverable_prompt = read_prompt('deliverable_prompt.txt')

        # Store all deliverables results
        all_deliverables = {
            "goal": all_impacts["goal"],
            "actors": []
        }

        # Get deliverables for each actor and each impact
        for actor in all_impacts["actors"]:
            ...
            for impact in actor["impacts"]:
                try:
                    # Build the data structure required for current analysis
                    current_data = {
                        "actor": {
                            "actor_id": actor["actor_id"],
                            "actor_name": actor["actor_name"]
                        },
                        "impact": {  # Modify to single impact
                            "impact_id": impact["impact_id"],
                    ...
                    while retry_count < max_retries and not success:
                        try:
                            ...
                            # Build the complete impact data, including deliverables
                            impact_data = {
                                "impact_id": impact["impact_id"],
                                "impact_description": impact["impact_description"],
                                "deliverables": [
                            ...
                            # Print current deliverable results
                            ...
                            if retry_count == max_retries:
                                ...
                                if retry_count == max_retries:
                                    # If all retries fail, use an empty deliverables list to continue
        ...
        # Print the final complete deliverables results
        print(f"\n{Fore.GREEN}Final combined deliverables:")
        print(json.dumps(all_deliverables, indent=2))
        print(Style.RESET_ALL)

        # Get the prompt template for user story
        userstory_prompt = read_prompt('userstory_prompt.txt')

        # Store all user stories results
        all_userstories = {
            "goal": all_deliverables["goal"],
            "actors": []
        }

        # Get user stories for each actor and each impact and each deliverable
            ...
            for impact in actor["impacts"]:
                ...
                while retry_count < max_retries and not success:
                    try:
                        ...
                        # Build the complete impact data, including all deliverables' user stories
                        impact_data = {
                            "impact_id": impact["impact_id"],
                            "impact_description": impact["impact_description"],
                            "deliverables": [
                        ...
                        # Print current user stories results
        ...
        # Print the final complete user stories results
        ...
        # Even if an error occurs, the previous results have been saved
        continue
